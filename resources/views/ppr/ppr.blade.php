...
<div class="pagetitle">
  <div class="row align-items-center">
    ...
    <div class="col">
      <div class="pagetitle">
        <h1>Probation Period Review</h1>
        <nav>
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{url('dashboard')}}">Home</a></li>
            <li class="breadcrumb-item active">Probation Period Review</li>
          </ol>
        </nav>
      </div>
    </div>
...
<!-- Search Filter -->
<!-- <form action="{{route('probation-period-review.emploees_filter')}}" method="post">
        @csrf
  <div class="row">
        ...
        @foreach($employees as $employee)    
          <tbody>
            <tr role="row" class="odd">
              ...
              <td>@if($employee->ppr) {{date('d-m-Y', strtotime($employee->ppr->review_date))}} @endif</td>
              <td>            
              ...
              <td>               
                ...
                @if($user->hasRole(VP_ROLE) || $user->hasRole('Principal') ||$user->hasRole('Executive-Admin') )
                  @if($employee->ppr)
                    <div class="action-btn bg-yellow-500 ms-2" style="margin-left: 0px !important;">
                      <a href="#" data-target=".ppr_modal" data-toggle="modal" onclick="editEmployeeDetails('{{$employee->ppr->id}}')"  data-id="{{$employee->id}}" >
                  ...
                  @else
                    <div class="action-btn bg-green-500 ms-2" style="margin-left: 0px !important;">
                      <a href="#" data-target=".ppr_modal" data-toggle="modal" onclick="getEmployeeDetails('{{$employee->id}}')"  data-id="{{$employee->id}}" >
                ...
                @endif
                  ...
                  <a href="#" data-target=".ppr_modal" data-toggle="modal" onclick="viewEmployeeDetails('{{$employee->ppr->id}}')"  data-id="{{$employee->id}}">
                        <i class="bi bi-eye"
              ...
              @if($user->hasRole(SUPER_ADMIN_ROLE))
                  ...
                  <div class="action-btn bg-red-500 ms-2">
                    <a href="{{route('ppr.delete',$employee->ppr->id)}}" >
...
<div id="ppr_modal" class="modal ppr_modal  custom-modal fade" role="dialog" style="display: none;" aria-hidden="true">
...
function getEmployeeDetails(employeeId) {
  // alert(employeeId);
        $.ajax({
            url: 'probation-period-review/get-employee-details',
            method: 'POST',
            data: {
...
//Appraisal edit modal
    function editEmployeeDetails(employeeId) {
        $.ajax({
            url: 'probation-period-review/edit-employee-details',
            method: 'POST',
            data: {
                employee_id: employeeId,
                _token:"{{csrf_token()}}",
...
function viewEmployeeDetails(employeeId) {
        $.ajax({
            url: 'probation-period-review/show',
            method: 'POST',
            data: {
                employee_id: employeeId,
                _token:"{{csrf_token()}}",
            ...
            success: function(response) {
                ...
                console.log(response);
                  $('#update-content').html(response);
                  // $('#ppr_modal').modal('show'); // Display the modal
