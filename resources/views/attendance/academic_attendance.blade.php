...
@php
    ...
    // Check if the current day is greater than or equal to 26th of the month
    if ($currentDate->day >= 26) {
        // Set the start date as the current date
        $startDate = $currentDate;
        
        // Set the end date as the 25th day of the next month
        $end_Date = $currentDate->copy()->addMonthsNoOverflow(1)->startOfWeek(Carbon::SUNDAY)->endOfWeek(Carbon::SATURDAY);
        
        // Set the start date as the 26th of the previous month
        $start_Date = $currentDate->copy()->subMonthNoOverflow()->setDay(26)->startOfWeek(Carbon::SUNDAY);
    } else {
        // Set the start and end dates for the current period
        $start_Date = $currentDate->copy()->subMonthNoOverflow()->setDay(26)->startOfWeek(Carbon::SUNDAY);
        $end_Date = $currentDate->copy()->setDay(25)->endOfWeek(Carbon::SATURDAY);
    }

    // Get the end date of the current week
    $endday = \Carbon\Carbon::parse($end_Date);
    $day = $endday->day;
    $dayInt = (int)$day;

    // Get the start and end dates of the previous week
    $prevStartDate = Carbon::parse($startDate)->subWeek()->startOfWeek(Carbon::SUNDAY);
    $prevEndDate = $prevStartDate->copy()->endOfWeek(Carbon::SATURDAY);

    // Get the start and end dates of the next week
    $nextStartDate = $end_Date->copy()->addDay()->startOfWeek(Carbon::SUNDAY);
    $nextEndDate = $nextStartDate->copy()->endOfWeek(Carbon::SATURDAY);

    // Get the number of days in the current week (7 days)
    $numDays = 7;
        ...
        <table class="table table-striped table-bordered custom-table table-nowrap mb-0">
            ...
            <tbody id="attendance-table-body">
                ...
                @foreach($acadmic_employees as $acadmic_employee)
                    <tr>
                        <td class="freeze-td" style="background-color:#dce0e1;">
                        ...
                        @while($loop_date <= $end_date)
                            ...
                            <td @if(date('N', strtotime($loop_date->format('Y-m-d'))) == 5 || date('N', strtotime($loop_date->format('Y-m-d'))) == 6) style="background-color: #dce0e1;" @endif style="z-index:-9999;">
...
<script>
    ...
    $(".next").click(function(e) {
        ...
        for (let i = 1; i <= numDays; i++) {
            ...
            if (currentDate.day() === 5) { 
                cell.css('background-color', 'grey'); // Highlight Fridays
            }

            if (currentDate.day() === 6) { 
                cell.css('background-color', 'grey'); // Highlight Saturdays
            }
                ...
                success: function(responseData) {
                    ...
                    if (responseData.data.length > 0) {
                        ...
                        if (attendance.status === 'check-in' && attendance.check_in != null) {
                                ...
                                const date = new Date(attendance.check_out);
                                const time = date.toLocaleTimeString('en-US', {
                                                hour: 'numeric',
                                                minute: '2-digit',
                                                second: '2-digit',
                        ...
                        $('.modal-body .statistics #break h6').html(totalBreakTime.toFixed(2)+' mins');  //'hrs'                
                        // Show the modal
