...
<main id="main" class="main">
    ...
    <!-- Search Filter -->
    @if ($edit_masters)
    <form action="{{ route('relationship.update', $edit_masters->id ) }}" method="post">
    {{ method_field('PUT') }}
    @else
        ...
        <div class="row filter-row mb-30">
            ...
            <div class="col-sm-6 col-md-3 col-lg-3 col-xl-4 col-12">
                <div class="form-group">
                    ...
                    <input name="relationship_name" placeholder="Enter Relationship" value="{{$edit_masters ? $edit_masters->name : ''}}" type="text" class="form-control">
                </div>
            </div>
            ...
            <div class="col-md-1" style="padding-top: 7px;">
                <a href="{{ url('relationship') }}" class="btn btn-block btn-danger">Cancel</a>
            </div>
            @endif
        </div>
        ...
        <div class="col-lg-12">
            <div class="table-responsive">
                <table class="table table-striped custom-table datatable dataTable-selector"
                    ...
                    <tbody>
                        @forelse ($masters_data as $relationship)
                        <tr role="row" class="odd">
                            ...
                            <td>
                                <a href="{{ route('relationship.status', ['id' => $relationship->id, 'status' => $relationship->active]) }}" class="badge rounded-pill badg-actv btn-{{$relationship->active ? 'success' : 'danger'}}">
                                    {{getStatus($relationship->active)}}
                                </a>
                            </td>
                            <td>
                                <div class="action-btn bg-cyan-500 ms-2">
                                    <a href="{{route('relationship.show', $relationship->id)}}">
                                        <i class="bi bi-pencil"
                                            style="font-size: 15px; display: flex; color: #ffffff;"></i>
                                    </a>
                                </div>
                                <!--<div class="action-btn bg-red-500 ms-3">
                                    <a data-toggle="modal" data-target="#delete_{{ $relationship->id }}">
                                        <i class="bi bi-trash"
                                            style="font-size: 15px; display: flex; color: #ffffff;"></i>
                                    </a>
                                </div>-->
    ...
    <!-- Delete Modal -->
    @forelse ($masters_data as $relationship)
    <div class="modal custom-modal fade" id="delete_{{$relationship->id}}" role="dialog">
            ...
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <div class="modal-btn delete-action">
                        <div class="row">
                            <div class="col-md-6">
                                <a href="{{route('relationship.delete', $relationship->id)}}"
