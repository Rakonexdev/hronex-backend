...
<main id="main" class="main">
                ...
                <div class="pagetitle">
                    ...
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">Paid Status</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <!-- Search Filter -->
    @if ($edit_masters)
    <form action="{{ route('paidstatus.update', $edit_masters->id ) }}" method="post">
    {{ method_field('PUT') }}
    @else
    <form action="{{ route('paidstatus.store') }}" method="post">
    @endif
        @csrf
        <div class="row filter-row mb-30">
            ...
            <div class="col-sm-6 col-md-3 col-lg-3 col-xl-4 col-12">
                ...
                <label class="form-label">Paid Status</label>
                    <input name="paidstatus_name" placeholder="Enter Paid Status" value="{{$edit_masters ? $edit_masters->name : ''}}" type="text" class="form-control">
                </div>
            </div>
            ...
            <div class="col-md-1" style="padding-top: 7px;">
                <a href="{{ url('paidstatus') }}" class="btn btn-block btn-danger">Cancel</a>
            </div>
            @endif
        </div>
        ...
        <div class="col-lg-12">
            <div class="table-responsive">
                <table class="table table-striped custom-table datatable dataTable-selector"
                    ...
                    <tbody>
                        @forelse ($masters_data as $paidstatus)
                        <tr role="row" class="odd">
                            ...
                            <td>{{$paidstatus->description}}</td>
                            <td>
                                <a href="{{ route('paidstatus.status', ['id' => $paidstatus->id, 'status' => $paidstatus->active]) }}" class="badge rounded-pill badg-actv btn-{{$paidstatus->active ? 'success' : 'danger'}}">
                                    {{getStatus($paidstatus->active)}}
                                </a>
                            </td>
                            <td>
                                <div class="action-btn bg-cyan-500 ms-2">
                                    <a href="{{route('paidstatus.show', $paidstatus->id)}}">
                                        <i class="bi bi-pencil"
                                            style="font-size: 15px; display: flex; color: #ffffff;"></i>
                                    </a>
                                </div>
                                <div class="action-btn bg-red-500 ms-3">
                                    <a data-toggle="modal" data-target="#delete_{{ $paidstatus->id }}">
                                        <i class="bi bi-trash"
                                            style="font-size: 15px; display: flex; color: #ffffff;"></i>
                                    </a>
                                </div>
    ...
    <!-- Delete Modal -->
    @forelse ($masters_data as $paidstatus)
    <div class="modal custom-modal fade" id="delete_{{$paidstatus->id}}" role="dialog">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body">
                    <div class="form-header">
                        <h3>Delete Paid Status</h3>
                        <p>Are you sure want to delete?</p>
                    </div>
                    <div class="modal-btn delete-action">
                        <div class="row">
                            <div class="col-md-6">
                                <a href="{{route('paidstatus.delete', $paidstatus->id)}}"
