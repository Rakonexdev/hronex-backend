...
<main id="main" class="main">
                ...
                <div class="pagetitle">
                    ...
                    <nav>
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">SCF Data</li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <!-- Search Filter -->
        @if ($edit_masters)
            <form action="{{ route('scf-data.update', $edit_masters->id ) }}" method="post">
            {{ method_field('PUT') }}
        @else
            <form action="{{ route('scf-data.store') }}" method="post">
                @endif
                @csrf
                ...
                <h4 class="mb-2 mt-2 bgh4">{{$edit_masters ? 'Edit' : 'Create'}}</h4>
                    ...
                    <div class="col-sm-8 col-md-6 col-lg-6 col-xl-4 col-12">
                        <div class="form-group">
                            <label class="form-label">SCF </label>
                            <textarea name="scf" type="text" placeholder="Enter scf" class="form-control txt-high">
                                {{$edit_masters ? $edit_masters->scf : ''}}
                            </textarea>
                        </div>
                    </div>
                    ...
                    <table class="table table-striped custom-table datatable dataTable-selector"
                        ...
                        <tbody>
                            ...
                            <tr role="row" class="odd">
                                ...
                                <td>{{$scf_data->scf}}</td>
                                <td>
                                    <span class="badge rounded-pill badg-actv btn-{{$scf_data->status == 1 ? 'success' : 'danger'}}">{{$scf_data->status == 1 ? 'Active' : 'In Active'}}</span>                                         
                                </td>
                                <td>
                                    <div class="action-btn bg-cyan-500 ms-2">
                                        <a href="{{route('scf-data.show', $scf_data->id)}}">
        ...
        <!-- Delete Modal -->
        @forelse ($scf_datas as $scf_data)
        <div class="modal custom-modal fade" id="delete_{{$scf_data->id}}" role="dialog">
