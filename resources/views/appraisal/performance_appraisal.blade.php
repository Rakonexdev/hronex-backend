      ...
      <table class="table table-striped custom-table datatable dataTable-selector" style="display: inline-table; padding: 5px;">
        ...
        @foreach($employees as $employee)    
          <tbody>
            ...
            <td>@if($employee->appraisalreprot) {{date('d-m-Y', strtotime($employee->appraisalreprot->evaluation_date))}} @endif</td>
              <td>            
                @if($user->hasRole('Principal') ||$user->hasRole('Vp')||$user->hasRole('Executive-Admin') )
                  @if($employee->appraisalreprot)
                    @if($employee->appraisalreprot->principal_rating_avg_compt != 0  ) 
                ...
                @elseif($user->hasRole('Hr')  )
                  @if($employee->appraisalreprot)
                    @if($employee->appraisalreprot->principal_rating_avg_compt != 0  )  
                      <i class="fa fa-dot-circle-o text-success" title="Reviewed"></i>
                    @elseif($employee->appraisalreprot->hod_rating != null)
                      <i class="fa fa-dot-circle-o text-warning" title="Principal Rating Pending"></i>
                    @else
              ...
              <td>               
                ...
                @if($user->hasRole(VP_ROLE) || $user->hasRole('Principal')||$user->hasRole('Executive-Admin'))
                    @if(!empty($employee->appraisalreprot->principal_rating_avg_compt))
                  ...
                  @if($employee->appraisalreprot)
                    <div class="action-btn bg-yellow-500 ms-2" style="margin-left: 0px !important;">
                      <a href="#" data-target=".edit_appraisal" data-toggle="modal" onclick="editEmployeeDetails('{{$employee->employee_id }}')"  data-id="{{$employee->id}}" >
                ...
                @endif
                  ...
                  <a href="#" data-target=".edit_appraisal" data-toggle="modal" onclick="viewEmployeeDetails('{{$employee->appraisalreprot->id}}')"  data-id="{{$employee->id}}">
                        <i class="bi bi-eye"
                  ...
                  <div class="action-btn bg-red-500 ms-2">
                    <a href="{{route('appraisalreport.delete',$employee->appraisalreprot->id)}}" >
...
function getEmployeeDetails(employeeId) {
        $.ajax({
            url: 'appraisal/get-employee-details',
            method: 'POST',
            data: {
...
//Appraisal edit modal
    function editEmployeeDetails(employeeId) {
        $.ajax({
            url: 'appraisal/edit-employee-details',
            method: 'POST',
            data: {
                employee_id: employeeId,
                _token:"{{csrf_token()}}",
